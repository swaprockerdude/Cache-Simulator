...
private:
  ...
  int isFull() { return cyclesUntilEmpty > maxCycles - memLatency;} 
	/**
	(maxCycles - memLatency) allows to check if space is available 
  for buffering at least one item
  */
};
...
public:
  ...
  */
  ...
  /**
	gets num of accesses to a cache blk
	*/
  int getrefcount();
 ...
 /**
	calculates num of accesses to a cache blk
 */
  int refCount;
};
...
public:
  ...
  */
  ...
  /**counts num of ref to a cache blk*/
  int refcnt(int addr);

  /**Function to swap two cache blocks*/ 
  int swap(int addr);


private:
  CacheBlock* blocks; CacheBlock* blocks1; CacheBlock* blocks2;
